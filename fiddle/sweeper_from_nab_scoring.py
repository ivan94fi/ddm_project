...
def scaledSigmoid(relativePositionInWindow):
    ...
    corresponds to S = 2*sigmoid(5) - 1.0 = 0.98661.
    ...
    right  edge of the  window and corresponds to a score of
    2*sigmoid(-5) - 1.0 = -0.98661.

    @param  relativePositionInWindow (float)  A relative position
                                              within a window calculated per
                                              the rules above.
...
class Sweeper(object):
    ...
    def calcSweepScore(
        ...
        for i, (curTime, curAnomaly) in enumerate(zip(timestamps, anomalyScores)):
            ...
            point = AnomalyPoint(curTime, curAnomaly,
                                 weightedScore, pointWindowName)
    ...
    def calcScoreByThreshold(self, anomalyList):
        ...
        for dataPoint in scorableList:
            ...
            if dataPoint.anomalyScore != curThreshold:
                ...
                s = ThresholdScore(curThreshold, curScore,
                                   tp, tn, fp, fn, totalCount)
